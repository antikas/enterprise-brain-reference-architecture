...
An **enterprise brain** is the institutional memory-and-reasoning system of an organisation: the system that **captures the organisation's proprietary knowledge, finds and assembles the right context for any request, resolves which facts are true and where they came from, reasons over them, and does all of that resiliently, securely, lawfully, valuably, and at scale.** It is the difference between an organisation that *has* documents and one that can *reason from what it knows*.
...
The evaluation framework asks, in seven quality lenses, *"is the brain a brain? is it trustworthy? resilient? secure? compliant? valuable? at scale?"* — forty-one measurable **dimensions**, each anchored to a recognised external standard. This architecture answers each of those dimensions with a capability that owns it. The **cross-walk** in §4 is the conformance instrument that proves the answer is complete: every evaluation dimension maps to a capability, and every capability traces back to the dimension(s) it satisfies. When the requirements change, the cross-walk is the drift detector that shows where the architecture must follow.

A reader who knows a concern from the evaluation framework — *"how does the brain stop a confident-but-wrong write from corrupting its memory?"* — can find the capability that owns it (here, `C-G3.2`) through the cross-walk, read its contract, and see exactly what the architecture promises.
...
1. A **frontier orchestrator** — a general, commoditised, swappable reasoning model that decomposes a request, plans, and answers anything novel. It is deliberately *never* specialised to the organisation, so it can always be replaced by a better one.
2. A **specialist inventory** — a patiently-accumulated set of smaller models, each tuned on a slice of the organisation's **proprietary corpus** to do one recurring thing better and cheaper than the frontier. This inventory is the **moat**: it is the part a competitor cannot buy, because it is grown from knowledge only this organisation holds.
...
- **Quality attributes are stated as classes, not numbers.** A capability declares it must meet an *availability class* or a *latency class*; each adopting organisation sets the actual target. The architecture never says "99.9%" — that is an instance decision.
...
Like the two-tier reasoning band (GX), the experience flywheel is not a single capability — it is a **cross-cutting expression** that runs *through* the groups. It is the brain's defining runtime property: **every production interaction is a learning event** that, captured and promoted under gate, makes the brain better for the next one. The loop is one always-on cycle:
...
| C-G2.2 Compose context for reasoning | deterministic-claim | — (supports A2) | A2(s), G2(s), B4(s) | P9, P5 |
...
| C-G6.3 Recover from store | deterministic-claim | C3 | — | P1, P13 |
...
| C-G6.7 Account for unit cost and budget | deterministic-claim | G1 + G2 | — | P5, P2 |
| C-G6.8 Meet latency objectives | deterministic-claim | G3 | — | P2, P11 |
...
**Structurally deferred at n=1** (declare the contract/gate now; enforcement bites at scale): access mediation as live multi-tenant traffic (`C-G4.1`), the multi-tier router (`C-X1`) and the populated specialist inventory (`C-X2`) — frontier-first is the honest n=1 stance, residency and operational-resilience obligations that bind an organisation rather than a person (`C-G5.6`), the availability and recovery *consequences* (`C-G6.1`, `C-G6.3` — declare the targets), latency / caching / sustainability as enterprise gates (`C-G6.8`, `C-G6.9`, `C-G6.10`), tenant isolation under load (`C-G6.12`), and the cross-user promotion boundary (`C-G5.7`) — the flywheel's cross-user scale needs a population to bite, so the consent/de-identification gate is declared now and enforced when the brain has many users.
