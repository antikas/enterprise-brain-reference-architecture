...
| ID | Capability | Component(s) on our stack | Status |
|---|---|---|---|
| C-G5.1 | Operate the AI management system | The maturity scorecard + healthcheck + lint as monitoring signals; breach→corrective-action policy | **PARTIAL** — scorecard: Evaluation = Emerging; signals exist and a halt-on-breach norm is declared, but there is **no enforced cadence** (no scheduled task fires the loop) — it fails its own forcing-function bar |
...
The honest reading: *capture everything, promote nothing unaudited* is realised today for the **single-learner / same-user** scales — the distil→eval-gate→promote→improve arc is built end-to-end at build-cycle grain. The **cross-user scale** (and its consent/de-identification boundary, `C-G5.7`) is the PLANNED remainder — the flywheel's next turn, deferred until the brain has a population.
...
2. **C-G3.1 source-of-truth = PARTIAL, marked *down* from a tempting BUILT.** A `decay_scorer` exists and the trust rule is articulated — it looks built. But `valid_from` autofill is unimplemented, so most files lack the recency anchor and decay is a ~1–2%-of-queries soft nudge, not "recency wins by rule". The scorecard grades it Partial; so it is PARTIAL here.
